...
`Math.sqrt(N)`の計算量は$\Omicron(\log N)$なので、ここをどうにかしたかったんです。 
...
$\Omicron\left(\frac{N}{\log N}\right)$から$\Omicron\left(\frac{N}{(\log N)^2}\right)$  
に減らせているはずなんです。  
...
これを使うことで、計算量を$\Omicron(\log\log N)$まで減らすことができます。  
